...
flowchart LR
  ...
  repo --> s3["Provider-qualified private object storage<br/>CID-addressed bytes"]
  backfill["Pre-activation closure backfill"] --> cannon["repo.usecannon.com / legacy IPFS source"]
  backfill -->|"CID-verified raw bytes"| s3
...
The three data planes have different durability and security contracts and must use separate production instances with separate credentials and network policy; key namespaces alone are not isolation. If a non-production environment temporarily shares a physical Redis service, each credential must be restricted by ACL to the service's exact key patterns and required commands, and an isolation test must prove that the package/indexer and artifact-service credentials cannot scan, read, write, delete, subscribe to, or administer Safe staging data. A shared instance is not an acceptable production posture for mainnet staging. The package API/indexer specifically requires Redis Stack features such as search and time series; plain Valkey compatibility is not sufficient without proving every used module.

| Service              | Current keys/role                                                                                                          | Consequence of loss                                                                                        | Required production posture                                                                                                                                                        |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Safe staging backend | `safe-app-backend:<chainId>-<safeAddress>` contains the serialized transaction map and accumulated signatures              | Staged proposals and collected signatures disappear after a restart or cache miss                          | Dedicated persistent Valkey with dedicated credentials, backups, encryption, restricted network access, restore drill, and no eviction                                             |
| Artifact repository  | Sorted sets `repo:tempUploadHashes`, `repo:pkgHashes`, and `repo:longTermHashes` authorize/index CIDs; S3 stores the bytes | S3-resident blobs remain readable, but promotion state and upload authorization are lost                   | Dedicated persistent Valkey plus deterministic rebuild from artifact-closure manifests; PRO-692 may remove these legacy sets only if authenticated writes no longer depend on them |
...
| -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST /api/v0/add` with multipart field `data`; also `POST /api/v0/add?local=true&to-files=/CID` with the same field | The website hook uses the no-query form; builder writes use the query form. The current repository handles both as the same regular upload: it recomputes the CID, accepts already-authorized bytes or inflated JSON with a CID-shaped `miscUrl`, stores bytes in S3, and returns `{"Hash":"<computed CID>"}`. It does not validate the full `DeploymentInfo` schema or enforce the `to-files` CID itself; the builder rejects a returned hash that differs from its locally computed CID, while the website consumes the returned `Hash` | Preserve both request shapes and the `data` field. Authenticate writers, enforce schema and size limits, require a supplied `/CID` to equal the independently computed CID, and always return the computed CID in the existing JSON shape |
| `POST /api/v0/cat?arg=CID`                                                                                           | Reads and CID-verifies S3; returns 404 when the CID is absent                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Preserve local-only reads and fail closed on missing or corrupt bytes                                                                                                                                                                     |
| `HEAD /api/v0/cat?arg=CID`                                                                                           | Checks S3 only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Preserve as the authoritative local-presence probe                                                                                                                                                                                        |

Folder uploads (`wrap-with-directory`) are a separate website-bundle feature and now fail closed with 501. Reya serves website bundles through Cloudflare, so the artifact facade has no Pinata dependency or credential.

### Public IPFS is a backfill input, not a runtime fallback

The isolated migration job may read `repo.usecannon.com` or another public gateway before activation. It copies exact bytes, verifies every CID, writes the verified closure to Reya storage, and then exits. The production service has no `IPFS_URL` configuration or network path to hosted Cannon. This makes activation depend on a complete backfill rather than the continued availability of infrastructure Cannon can change, deprecate, rate-limit, or stop operating.
...
6. Assert that live registry resolution still matches the snapshot, discover the authoritative manifest without Redis by walking the validated chain from genesis, then rebuild the artifact Redis indexes from it on an empty database. Compare exact sorted-set membership, classification, scores and provenance to the manifest, and exercise an isolated authorization operation whose result depends on each declared durable index class rather than S3 presence alone. Race competing publishers and inject crashes on both sides of the commit point; prove that only the winning child extends the chain, preparations are ignored, and retries converge with a stale/deleted head cache and empty Valkey.
7. Prove there is no runtime hosted-IPFS configuration or egress path at the repository, browser and CSP/network-policy layers; restart the facade to clear memoized results; run real builder/CLI and package/source/staging browser flows; raw-fetch every CID listed in the artifact-closure manifest, recompute its CID and compare its byte length.
...
9. Test the newest package written after cutover with the primary facade unavailable. Hosted Cannon is not a runtime failover for Reya writes. If independent failover is required during the rollback window, dual-copy and verify every new CID to that target.
...
> 3. Is there a supported offline way to backfill and verify the full root + `miscUrl` + recursive import closure plus non-empty registry `metaUrl` blobs into a local-only Kubo-compatible repo facade backed by S3/Redis?
