...
- The **artifact-closure manifest** records the registry contract/network/block snapshot; each package resolution and whether that resolution is mutable; resolved deploy/meta URLs; every root/misc/meta/import CID and role; byte length; intended durable artifact-index class; source/destination; and verification evidence. Its content digest is the recovery identity.
- The **self-host release manifest** records upstream and Reya source SHAs, package/config-schema/toolchain versions, UI asset digest, image digests, and the exact artifact-closure-manifest digest promoted with that release.
...
PRO-692 must close these gaps or wrap the service with an equivalent hardened implementation while preserving the CID and HTTP contracts above. If it retains the legacy sorted sets, the canonical target mapping is: all deployment-info roots (including recursive imports) in `repo:pkgHashes`; all `miscUrl`/`metaUrl` support blobs in `repo:longTermHashes`; no finalized artifact-closure-manifest CID in `repo:tempUploadHashes`; and a score equal to the earliest retained referencing registry block timestamp in seconds. An unknown CID may be served only when present in S3; upstream fallback must reject CIDs absent from the permanent manifest-derived sets. The publish commit protocol must stage verified S3 objects, write an immutable artifact-closure-manifest version, transactionally update Redis indexes/current-manifest pointer, and report success only after all steps; interruption must be idempotently replayable. If PRO-692 instead removes these indexes, it must prove that authenticated writes, local reads and fallback policy no longer depend on them.

After the newest post-cutover publish, the recovery test must destroy Valkey, rebuild solely from artifact-closure manifests, assert exact members/classes/scores (or prove the replacement has no such state), and exercise behavior that depends on each class rather than S3 presence alone.
...
5. Produce a versioned artifact-closure manifest containing package reference, chain ID, registry snapshot, resolution mutability, resolved deploy/meta URLs, each CID's root/misc/meta/import role, intended durable artifact-index class, byte length, source, destination and verification time.
6. Assert that live registry resolution still matches the snapshot, then rebuild the artifact Redis indexes from the artifact-closure manifest on an empty database. Compare exact sorted-set membership, classification and scores to the manifest, and exercise an isolated authorization/fallback operation whose result depends on each declared durable index class rather than S3 presence alone.
7. Disable fallback at the repository, browser and CSP/egress layers; restart the facade to clear memoized results; run real builder/CLI and package/source/staging browser flows; raw-fetch every artifact-closure-manifest CID, recompute its CID and compare its byte length.
