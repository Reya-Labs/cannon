...
| Artifact repository | Sorted sets `repo:tempUploadHashes`, `repo:pkgHashes`, and `repo:longTermHashes` authorize/index CIDs; S3 stores the bytes | S3-resident blobs remain readable, but promotion state, upload authorization, and upstream-fallback classification are lost | Dedicated persistent Valkey plus deterministic rebuild from artifact-closure manifests; PRO-692 may remove these legacy sets only if authenticated writes and fallback behavior no longer depend on them |
